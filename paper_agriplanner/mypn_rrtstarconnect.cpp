#include "mypn_rrtstarconnect.h"
#include "ompl/base/goals/GoalSampleableRegion.h"
#include "ompl/tools/config/SelfConfig.h"
#include "ompl/util/String.h"

ompl::geometric::RRTStarConnect::RRTStarConnect(const base::SpaceInformationPtr &si, bool addIntermediateStates) : base::Planner(si, addIntermediateStates ? "RRTConnectIntermediate" : "RRTConnect") {
    specs_.recognizedGoal = base::GOAL_SAMPLEABLE_REGION;
    specs_.directed = true;

    Planner::declareParam<double>("range", this, &RRTStarConnect::setRange, &RRTStarConnect::getRange, "0.:1.:10000.");
    Planner::declareParam<bool>("intermediate_states", this, &RRTStarConnect::setIntermediateStates, &RRTStarConnect::getIntermediateStates, "0,1");
...
ompl::geometric::RRTStarConnect::GrowState ompl::geometric::RRTStarConnect::growTree(TreeData &tree, TreeGrowingInfo &tgi, Motion *rmotion) {
...
ompl::base::PlannerStatus ompl::geometric::RRTStarConnect::solve(const base::PlannerTerminationCondition &ptc) {
    ...
    while (!ptc) {
        ...
        if (gs != TRAPPED) {
            ...
            if (gsc == REACHED && goal->isStartGoalPairValid(startMotion->root, goalMotion->root)) {
                ...
                solved = true;
                break; // solution is found. so they break out of the loop here!
    ...
    OMPL_INFORM("%s: Created %u states (%u states on start tree + %u states on goal tree)", getName().c_str(), tStart_->size() + tGoal_->size(),
