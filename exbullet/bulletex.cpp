...
int main(int argc, char **argv) {
    ...
    btCollisionShape *groundShape = new btStaticPlaneShape(btVector3(0, 1, 0), 1); // Flat ground on Y-axis
    ...
    btDefaultMotionState *fallMotionState = new btDefaultMotionState(btTransform(btQuaternion(0, 0, 0, 1), btVector3(0, 50, 0))); // Start 50 units high
    ...
    for (int i = 0; i < 300; i++) {
        dynamicsWorld->stepSimulation(1 / 60.f, 10); // Step the simulation (1/60 second per step)

        btTransform trans;
        fallRigidBody->getMotionState()->getWorldTransform(trans); // Get cube position
