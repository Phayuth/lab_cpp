...
class Player : public Person {
    ...
    public:
        ...
        Player(const std::string &firstname, const std::string &lastname) : Person(firstname, lastname) {
        };
...
int main(int argc, char const *argv[]) {
    Person p1;
    printf("Person : %s %s, %s\n", p1.firstname.c_str(), p1.lastname.c_str(), p1.job.c_str());

    Player p2("John", "Wick");
    printf("Player : %s %s, %s\n", p2.firstname.c_str(), p2.lastname.c_str(), p2.job.c_str());
    printf("Player Person : %s %s, %s\n", p2.Person::firstname.c_str(), p2.Person::lastname.c_str(), p2.Person::job.c_str());
