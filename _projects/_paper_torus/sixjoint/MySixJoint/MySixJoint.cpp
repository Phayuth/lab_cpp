...
int main(int argc, char* argv[]){
    ...
    b3LoadUrdfCommandSetStartPosition(command, 0.0, 0.0, -0.005); // offset the plane down abit to avoid collision with robot base
    statusHandle = b3SubmitClientCommandAndWaitStatus(kPhysClient, command);
    statusType = b3GetStatusType(statusHandle);
    ...
    {
        bool col = collision_check_per_config(kPhysClient, ur5eid, obstacles, jointIndices, target);
