...
std::vector<double> getCircleTarget(const std::vector<double> &pose,
                                    double timestep, double radius = 0.075,
                                    double freq = 1.0) {
...
int main(int argc, char *argv[]) {
    // Setup parameters
    std::string robot_ip = "192.168.0.3";
    double rtde_frequency = 500.0;    // Hz
    double dt = 1.0 / rtde_frequency; // 2ms
    uint16_t flags = RTDEControlInterface::FLAG_VERBOSE |
                     RTDEControlInterface::FLAG_UPLOAD_SCRIPT;
    ...
    RTDEControlInterface rtde_control(
        robot_ip, rtde_frequency, flags, ur_cap_port, rt_control_priority);
    RTDEReceiveInterface rtde_receive(
        robot_ip, rtde_frequency, {}, true, false, rt_receive_priority);
    ...
    try {
        while (running) {
            ...
            std::vector<double> servo_target =
                getCircleTarget(actual_tcp_pose, time_counter);
